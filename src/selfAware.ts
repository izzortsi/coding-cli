/**
 * Self-Awareness — coding-cli introspection and self-modification support.
 *
 * Detects coding-cli's own installation root (distinct from the project
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (fs.existsSync(pkgPath)) {
      try {
        ...
        if (pkg.name === 'coding-cli') return dir;
      } catch {
...
export function validateSelf(groveRoot: string): string {
  ...
  if (result.status === 0) return 'No TypeScript errors found in coding-cli source.';
