/**
 * Ollama Fetch Wrapper — Anthropic SDK <-> OpenAI-compatible API translation
 *
 * Intercepts Anthropic SDK requests, transforms to OpenAI format,
 * sends to Ollama's OpenAI-compatible endpoint, and translates response
 * back to Anthropic format.
 *
 * Based on zaiFetch.ts — Ollama uses the same OpenAI-compatible protocol
 * but requires no API key (local inference).
 */

import { randomUUID } from 'node:crypto';

export function createOllamaFetch(baseURL: string): typeof globalThis.fetch {
  return async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    // Send to Ollama — no auth needed (local inference)
    ...
    if (!response.ok) {
      ...
      process.stderr.write(`[ollama] HTTP ${response.status}: ${errorText.slice(0, 500)}\n`);
    ...
    try {
      ...
      process.stderr.write(`[ollama] Failed to parse response: ${rawText.slice(0, 500)}\n`);
      openaiResponse = {};
    }

    if (process.env.OLLAMA_DEBUG) {
      process.stderr.write(`[ollama] Raw response: ${rawText.slice(0, 1000)}\n`);
    }
    ...
    if (openaiResponse.error) {
      ...
      process.stderr.write(`[ollama] Model error: ${errMsg}\n`);
      const errorResponse = {
        id: `msg_${randomUUID().substring(0, 12)}`,
        type: 'message',
        role: 'assistant',
        content: [{ type: 'text', text: `[Ollama error: ${errMsg}]` }],
        model: anthropicBody.model,
...
function transformResponse(openaiResponse: any, model: string): any {
  ...
  if (choice) {
    // Accept any non-empty string content (including whitespace-only)
    ...
    process.stderr.write(`[ollama] No choices in response (keys: ${Object.keys(openaiResponse).join(', ')})\n`);
  }

  if (content.length === 0) {
    content.push({ type: 'text', text: '[Empty response from Ollama — try OLLAMA_DEBUG=1 for details]' });
  }
